...
- **2026-06-01 — Live v1.0.206 chart-object parity release.** The v1.0.205 Tools cohort now shares hit/select metadata, p1/p2/p3/anchor dragging, settings-dialog controls, derived-field refreshes, and hotkey-4 Blue Laser arming/halos: trendAngle, regression, flatChannel, disjoint, fibChannel, fibTimeZone, fibCircles, gannBox, gannFan, barPattern, avp, and triangle. This ships through the canonical public `/play` source and adds no broker, signing, fund movement, on-chain write, or SDK-order route.
- **2026-06-01 — Local v1.0.205 chart-tool cohort built.** The remaining scaffolded Tools cohort now commits, previews, and renders in the main game: trendAngle, regression, flatChannel, disjoint, fibChannel, fibTimeZone, fibCircles, gannBox, gannFan, barPattern, avp, and triangle. This is local/vault source only until the next public `/play` push, and it adds no broker, signing, fund movement, on-chain write, or SDK-order route.
...
| M3 | Build apps (Workbench rebuild) | 🟡 partial · live v1.0.206 finishes parity behavior for the scaffolded chart-tool cohort and keeps SDK-first MACD / Bollinger Bands / ATR + Parallel Channel campaign routes; Workbench tab restores still open (updated 2026-06-01) |
