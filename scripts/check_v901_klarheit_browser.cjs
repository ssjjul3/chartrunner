...
(async () => {
  ...
  console.log('\n-- A6 · Live-Weltband — in v1.0.904 (S5a.3) ins Control Center verlagert --');
  // Das schwebende Band auf der #stage ist weg; sein Echtgeld-Zustand wohnt
  // jetzt in der CC-Zustandszeile (#crCCArmState). Hier nur noch die Gegen-
  // probe, dass die Stage frei bleibt — die Zeile selbst prueft v904.
  let band = await page.evaluate(() => {
    localStorage.setItem('cr_arm_v1', '1');
    crArmGrammar.sync();
    const b = document.getElementById('crLiveBand');
    const line = document.getElementById('crCCArmState');
    return { there: !!b, ccLine: !!line };
  });
  check('globales ARM → KEIN Band mehr in #stage (ins CC verlagert)', band.there === false, band);
  check('die CC-Zustandszeile (#crCCArmState) existiert als neue Heimat', band.ccLine === true, band);
