...
- Latest public milestone: `2026-06-09` unified cursor/scroller backport live as `v1.0.230`; Runner is folded into the Scroller toolbar cycle with avatar-sprite display only while selected, non-runner scrollers holster Runner/laser input, COACH.llm stays headbar-routed with the toolbar launcher hidden, and Tools, Primitives, Blue Laser, and Alarm modes keep the shared `crLaserCursor` dotted source-to-cursor guide. `v1.0.223`-`v1.0.225` chart interaction/guidance parity and the v1.0.226-v1.0.228 broker-wheel/timeframe/coin backports remain included. Bot Terminal remains archived from public launchers/window.
