...
[![Phase](https://img.shields.io/badge/phase-v1.0.123%20%C2%B7%20M0.5%20active-success)](#status)
...
> **Recent — 2026-05-26** ([full record](CONSOLIDATED_STATUS_2026-05-26.md)): Marketplace UI wired to real on-chain listings (+ a buy drift fix) and a Phase-A PvP match lobby (`init_match`/`join_match`) both **shipped live**. **Name Register, resale/royalty, and `record_run` oracle-cite** are **written** and fold into **one batched registry re-upgrade** (also closes the M0.5 deploy-parity gap) — see [`docs/architecture/registry-upgrade-vX-deploy.md`](docs/architecture/registry-upgrade-vX-deploy.md). **x402** (bot access) planned + scaffolded — bidirectional, multi-rail, user-picks-how-to-pay — see [`docs/architecture/x402-integration-plan.md`](docs/architecture/x402-integration-plan.md). ⚠️ The claim-handle / resell / verified-run UI errors until the batched registry deploy lands.
...
| **M1** | Tokenomics paper + fiat onramp | $CHART (was $CHART) / $RUN supply curves, sinks, vesting, swap math. Restores the token UI surfaces (Profile balances, Marketplace, Missions) hidden behind v0.9.12 feature flags. **MoonPay** or **Coinbase Onramp** for fiat → $RUN distribution. **🟡 numbers committed 2026-05-25/26** (`docs/TOKENOMICS-PAPER.md` v0.2): $CHART→$CHART, algorithmic-scarcity issuance, recycle leaks, reserve-float rate, 70M reserve, 70/7/7/8/5/3 allocation (O-1/2/3/7/13/15/17). **Next:** O-4 entry-fee schedule, then onramp provider + UI flag-flip. |
...
| **M2.5** | SDK extraction (Phase 1 unblock) | M1.4 / M1.5 — extract `ChartRunnerSDK` from the inline IIFE into the publishable `@chartrunner/core` npm package via the inline-bundler pattern (see `sdk-m1-scaffold/DEFUNCT.md`). Unlocks Dexscreener / TradingView host overlays. **🟡 down-payment shipped 2026-05-26:** a versioned `window.ChartRunner` automation namespace (`automationVersion 1.0`; live `sdk` instance + `setSeed`/`getRunSummary`/`step()`/`setHeadless`) is live on `/play` — the same surface `@chartrunner/core` will export. Full agent-readiness assessment + remaining tiers (semantic input API, `on(events)`, replay layer, RPC bridge) in `docs/architecture/BOT-FRIENDLINESS.md`. |
| **M2.6** | Wallet identity · NFT avatars · Name Register | v0.9.95 ships the offline picker (top 20 Solana collections, curated SVG samples) so the demo never blocks on RPC. M2.6 wires **Helius DAS API** to read the connected wallet's actual NFT holdings and filter the picker to what the player owns. Extends `drawPlayer` with an image-source branch so the chosen NFT becomes the in-game avatar sprite, not just the Profile portrait. **Adds on-chain Name Register**: each wallet claims one unique runner name via `chartrunner_registry` (TokenProfile entity, PDA-keyed by name → global uniqueness enforced by Solana itself). First-connect modal: pick name → availability check → on-chain claim → name shown wherever the lite profile reads from `profileName`. Sets up M3 (NFT-gated bots), M4 (NFT-as-asset listings), and M8 (named players in tournament brackets). |
| **M3** | Build apps | Bot Terminal back online, Workbench **Bots / Strategies / Indicators / Terminal / Backtest / App Builder / Theme** tabs restored (Bots was archived in v1.0.30 alongside the v0.9.12 Build-phase cohort; in-game bot equip + orbital orb render path stays intact behind the gate). App Builder template gallery rewritten — the v0.9.11 P&L Tracker + Trade Notes templates are deprecated. **Metaplex Agent Kit** registers Workbench bots as on-chain agents (014 registry); **Coinbase x402** for paid API access on premium bots / advanced indicators. |
