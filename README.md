...
| **1** | SDK pull-over · drop runtime onto external chart hosts | Prototype runtime bundled in-game. Standalone `@chartrunner/core`, generated `/sdk/` artifacts, host overlays, and live adapters remain gated until package-readiness checks pass. |
...
| **M2.5** | SDK extraction (Phase 1 unblock) | The prototype demonstrates the SDK runtime boundary in-game. The standalone package, generated browser artifacts, host overlays, live DEX adapters, and wallet approval UX stay gated until the package is stable, tested, documented, and intentionally released. |
...
| **M6** | AI and Telegram | Public demo surfaces only; hosted agent transport is gated/private. |
