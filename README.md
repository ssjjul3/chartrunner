...
| **M2.6** | Wallet identity · NFT avatars | v0.9.95 ships the offline picker (top 20 Solana collections, curated SVG samples) so the demo never blocks on RPC. M2.6 wires **Helius DAS API** to read the connected wallet's actual NFT holdings and filter the picker to what the player owns. Extends `drawPlayer` with an image-source branch so the chosen NFT becomes the in-game avatar sprite, not just the Profile portrait. Sets up M3 (NFT-gated bots) and M4 (NFT-as-asset listings on the registry). |
