...
**The realtime-multiplayer thesis: MagicBlock.** Two players running on the same chart in real time isn't possible at Solana's base-layer 400ms slot times. **Ephemeral Rollups** solve this: the `MatchState` PDA gets delegated to MagicBlock's ER validator, where every player's `tick_player` instruction settles in **~1ms at zero fee**. Every 30s the rollup `commits` state back to the Solana base layer for verifiability — and at match end, `commit_and_finish` undelegates so the final state lives on Solana forever, naturally chaining into a `chartrunner_registry::record_run` for the on-chain leaderboard. Two thesis-mapping primitives, two ecosystem partners — **Phoenix for trades, MagicBlock for matches.** See `docs.magicblock.gg` and the integration scaffold at [`solana-connect/src/lib/magicblock-ephemeral.ts`](solana-connect/src/lib/magicblock-ephemeral.ts).
