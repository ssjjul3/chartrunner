...
| **M2 Coach AI** | 🔵 QUEUED · 0/11 | `crCoach.reply()` keyword/state matching + live ATR/score readout already ship (v1.x); the **v2 model-backed chat path** is the actual M2 scope and is unbuilt | 🟦 **Mostly in sync** — add a note that v1 Coach groundwork is live |
...
| **M3 Build apps** | 🔵 QUEUED · 0/16 | App Builder archived in v0.9.12; nothing shipped toward Workbench rebuild | ✅ **In sync** |
...
| **M6 AI · Telegram** | 🔵 QUEUED · 0/10 | Hermes gateway exists (crash-loop); no in-game bot bridge | ✅ **In sync** |
...
| **M11 Umbrel-native quant toolset** | 🔵 QUEUED · 4/13 (added 2026-05-28) | Strategic docs (COMPETITIVE_MAPPING + P0_REQUIREMENTS + DATA_MODELS) + `data_access/` Python package + `backtesting/engine/` runner + bootstrap — all Hermes-built at `/opt/data/chartrunner/` on Umbrel container. Remaining: 4 P0 views (Scanner CLI / Chart / Strategy Lab / Backtest Results) + canonical OHLC choice + cross-product contract + smoke test | ✅ **In sync** (created from the buildout, not retrofitted) |
...
| **M14 Bot-first runtime** | 🟢 BONUS · 0/10 (added 2026-05-28) | Newly added. Bot-first runtime + Agent Command Center: on-chain `BotBacktestRecord` PDA + extended `window.ChartRunner` SDK + unified **Bot Terminal** UI (Console + Chat + Agents) replacing the Coach tab + multi-agent adapter (Hermes/OpenClaw/Grok/Claude/Codex/Telegram) + 4 sample bots + Backtest History Viewer. Builds on the shipped `window.ChartRunner` namespace (2026-05-26). **Absorbs M2 Coach v2 + expands M6 AI/Telegram** — both redirect headers added | ✅ **In sync** (created from session-end strategic capture) |
...
**Headline:** 12 of 15 milestone docs are in sync with committed work. The exceptions are **M2.6** (major), **M2.5** (in flight, mislabelled QUEUED), and a minor note on **M2**. Plus two cross-cutting doc-hygiene items: the **stale version banner** and **this audit file** (the prior 05-18 version). M11 is newly added 2026-05-28 to track the Hermes-built Umbrel-native toolset that wasn't on the original roadmap; M12 is newly added 2026-05-28 as a bonus infra sidequest (Umbrel stack adoption — observability + scraper + dev accelerators).
